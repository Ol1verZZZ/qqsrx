...
//        json2.put("t", 2);
    ...
    public String getMovieById(HttpServletRequest request) {
        ...
        try {
            ...
            JSONObject json = new JSONObject();
            json.put("posterURL", "http://www.yylp.xyz/movie_pic/1.jpg");
            json.put("chName", movie.getChineseName());
            ...
            for(Casting casting: castingList){
                ...
                artistJson.put("imgsrc", "http://www.yylp.xyz/actor_pic/1.jpg");

                for(int i=1; i<=6; i++){
                    artistList.add(artistJson);
                }
            }
            json.put("actor_list", artistList);

            for(int i=1; i<=6; i++){
                imgList.add("http://www.yylp.xyz/pic_in_movie/" + i + ".jpg");
            }
